...
User::User(string userName) /* TODO(student): initialize */ 
  ...
  if (userName=="" || userName.at(0) < 97 || userName.at(0) > 122) {
    throw std::invalid_argument("user constructor: invalid parameter values");
  }
  for (unsigned int i = 0; i < userName.length(); i++) { //cannot contain uppercase
...
void User::addUserPost(Post* post) {
  ...
  if (post == nullptr) {
    throw std::invalid_argument("addUserPost: invalid parameter values");
  }
